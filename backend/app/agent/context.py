    ...
    try:
        ...
        if target_type in ("shot_image", "shot_video"):
            ...
            return (
                ...
                f"转场类型：{shot.transition_type or 'hard_cut'}\n"
                f"起始状态：{shot.start_state or '无'}\n"
                f"结束状态：{shot.end_state or '无'}\n"
                f"画面方向/空间轴线：{shot.screen_direction or '无'}\n"
                f"连续性约束：{shot.continuity_notes or '无'}\n"
                f"是否使用上一镜尾帧：{'是' if shot.use_prev_last_frame else '否'}\n"
                f"连续性刷新提示：{shot.continuity_dirty_reason if shot.continuity_dirty else '无'}\n"
                f"当前状态：{shot.state}\n"
