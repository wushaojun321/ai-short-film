...
9. 人物只输出主角、重要配角、关键反派、反复出现且有剧情功能的角色；无名士兵、通信兵、驾驶员、百姓、临时军官等功能性一次性角色不要建人物资产。
10. 场景只输出跨集复用场景或强剧情功能核心场景；一次性过场、普通室内外、单场动作发生地不要建场景资产。
11. 道具只输出贯穿多集或推动剧情的标志性道具；普通武器、普通装备、车辆、地图、报纸、粮食、旗帜等只在反复出现或成为剧情关键物时才建资产。
12. 不要为每个资产包额外输出“常规状态”variant；只有发生实质视觉变化时才输出 variant。
13. 对不建资产的角色、场景、道具可输出 ignore 行说明原因；不要为了完整列举而输出资产行。
14. 资产注册表是制作级关键资产清单，不是每集出场元素清单；宁可少而准，后续镜头提示词可直接描述一次性元素。
