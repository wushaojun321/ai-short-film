...
**总体要求**：所有资产图必须清晰可辨、可作为后续分镜和视频生成的参考图。整体风格偏**超现实电影质感**：真实摄影基础、梦境般光影、高级电影美术、细腻材质和强氛围，但不能变成动漫、插画、游戏CG或3D建模。人物资产必须是**真人演员超现实电影定妆参考照 / 影视棚拍参考照**。场景资产必须是**超现实影视场景参考照**，道具资产必须是**超现实真实产品摄影参考照**。Seedream 默认生成同质化较高，**请务必写得非常具体**，尤其是外貌特征、真实材质、光线和摄影质感，越具体越不容易生成相似图像。

**语言要求**：所有 `prompt` 必须使用中文撰写，保留剧本中的中文角色名、场景名、台词和风格词，不要翻译成英文。

### 人物角色 prompt 规范
必须包含以下所有维度，缺一不可：
1. 构图：真人演员超现实电影定妆参考图，竖屏9:16，同一位演员同一套造型。注意：面部特写、全身正面形象、侧面视角会在生图任务中拆成三张独立图片，资产 prompt 不要要求三视图拼在同一张图里。
...
12. 超现实电影质感要求：必须写入"真人演员定妆照、真实皮肤纹理、自然毛孔、真实织物、梦境般电影光影、高级电影美术、超现实氛围"
...
示例格式：
"竖屏9:16，真人演员古装超现实电影定妆参考图，同一位演员同一套造型，中性灰摄影棚背景，[角色名]，[适用场景/剧情阶段/服装状态]，[性别]，[脸型][肤色][骨相]，[五官细节]，[发型]，身穿[服装细节]，[配饰/伤势/道具状态]，气质[气质]，真实皮肤纹理，自然毛孔，真实织物，梦境般电影光影，高级电影美术，超现实氛围。排斥：[排斥项]，不要动漫风、不要插画风、不要游戏CG、不要3D建模、不要塑料皮肤、不要过度磨皮，不要三宫格，不要分屏"
...
示例格式：
"竖屏9:16，无人物，超现实影视场景参考照，[空间类型]，[构图]，[光线]，[氛围]，[关键陈设细节]，真实材质，梦境般电影光影，高级电影美术，避免插画感、游戏场景、3D渲染"
...
示例格式：
"竖屏9:16，超现实真实产品摄影参考照，[物品名称]，[材质][颜色]，[纹理细节]，[尺寸感]，[状态]，真实材质，梦境般棚拍光，高级电影美术，避免插画、游戏道具、3D建模"
...
7. prompt 必须是中文超现实电影质感参考提示词：真实摄影基础、梦境般光影、高级电影美术、细腻材质和强氛围；禁止动漫、插画、游戏CG、二次元、卡通、3D建模。
8. prompt 必须明确写出“沿用同一人物资产包的共享面部基准”，同一角色不同造型只改变服装、妆发、伤势、道具和场景状态；不要要求三视图拼在同一张图里。
9. 场景只提取反复出现或对剧情功能重要的场景；道具只提取关键标志性道具。
    ...
    "characters": [
      {
        ...
        "view_requirements": "面部特写、全身形象、侧面视角",
        "description": "剧情层面描述",
        "prompt": "中文超现实电影质感定妆参考提示词",
        "voice_profile": "固定音色与说话基调"
      }
    ],
    "scenes": [{"name": "场景名", "description": "剧情层面描述", "prompt": "中文超现实影视场景参考提示词"}],
    "props": [{"name": "道具名", "description": "剧情层面描述", "prompt": "中文超现实真实道具摄影参考提示词"}]
  }
}""",
    "user_prompt_template": "全剧规划：\n{series_context}\n\n分集规划：\n{episode_plan}\n\n原文块索引：\n{script_index}",
...
提示词规范：
- 总体风格：必须偏超现实电影质感，真实摄影基础、梦境般光影、高级电影美术、细腻材质、强氛围；不要普通白底证件照感，也不要动漫/插画/游戏CG。
- 人物：必须写成"真人演员超现实电影定妆参考照 / 影视棚拍参考照"，同一位演员同一套造型；明确适用场景、剧情阶段、服装/伤势/道具状态、脸型、肤色、骨相、发型、配饰、气质、真实皮肤纹理、自然毛孔、真实织物、梦境般电影光影、排斥项。不要要求面部、全身、侧面拼在同一张图里，生图任务会拆成三张独立视角图。
- 人物人脸一致性：如果资产描述提供了“人物资产包”和“共享面部基准”，positive_prompt 必须完整保留共享面部基准，并明确“与同一人物资产包内其他造型保持同一张脸、同一骨相、同一五官比例”；只能改变当前造型、服装、妆发、伤势和道具状态
- 场景：必须写成"超现实影视场景参考照"，明确空间、光线、时间、氛围、视角、真实材质，不要写白色纯色背景
- 道具：必须写成"超现实真实产品摄影参考照"，明确材质、颜色、状态、细节、真实磨损和梦境般棚拍光影
- 禁止使用或延续这些词：立绘、设定图、游戏角色、游戏CG、CG、二次元、动漫、卡通、插画、3D建模、塑料皮肤、过度磨皮、三宫格、分屏、多视角拼图
...
- 如果是人物资产，positive_prompt 只描述稳定身份、造型和超现实电影质感，不要把"面部特写、全身正面形象、侧面视角"写成同图三视图；实际生成时后端会分别追加单视角指令生成三张独立图片
