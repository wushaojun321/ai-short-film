...
目标：只做片段结构规划，不写完整分镜细节，不输出镜头列表，避免输出过长。

规划原则：
1. 忠实还原本集原文，不新增剧情，不改写台词。
2. 按剧情功能拆成 2-6 个片段：建立、试探、冲突、反应、转折、过渡、悬念等。
3. 每个片段只给出片段级信息，不要输出 shots，不要输出 shot_code，不要写镜头描述。
4. source_excerpt 必须是本片段对应的原文，尽量精简但保留关键台词。
5. 资产只引用 key_asset_ids，可同时保留人物/场景名方便人工阅读；不要复制资产描述、图片地址或长提示词。
6. 每个片段都要写清与上一片段、下一片段的衔接方向。

时长规则：
- 单镜头上限为 {max_shot_duration} 秒，但不是固定时长。
- 建立/关系镜一般 5-8 秒，台词镜 4-6 秒，动作镜 3-5 秒，反应/悬念镜 2-4 秒。
- 每个片段 target_duration 只给估算秒数。

只输出 JSON 对象，不要解释。顶层必须包含 segments 数组。
每个 segment 只能包含：segment_code、segment_name、segment_function、scene、characters、source_excerpt、transition_in、transition_out、target_duration、key_asset_ids。
禁止输出 shots、shot_list、storyboard、description、dialogues、required_assets。
key_asset_ids 只填写可用资产索引中的 id。""",
