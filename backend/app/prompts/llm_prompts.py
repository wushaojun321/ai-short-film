...
#   character_name / scene_scope / appearance_stage / view_requirements - 仅人物资产需要，用于区分同一角色不同场景和阶段的造型资产
# =============================================================================
SCRIPT_PARSE = {
...
- 同一角色在不同场景或不同阶段出现明显服装、伤势、身份状态、妆发、道具变化时，必须拆成独立人物资产
- 资产命名必须体现角色本名、场景/阶段和造型，例如：`李云湘-朝堂掌权期-深绛宫装`、`谢风凌-战损阶段-黑衣负伤`
- 每个人物资产必须填写 `character_name`、`scene_scope`、`appearance_stage`、`view_requirements`
...
### 人物角色 prompt 规范
必须包含以下所有维度，缺一不可：
1. 构图：真人演员定妆参考三视图，竖屏9:16，同一位演员同一套造型，必须同时包含：面部特写、全身正面形象、侧面视角，干净摄影棚背景或中性灰背景
...
示例格式：
"竖屏9:16，真人演员古装定妆参考三视图，同一位演员同一套造型，画面包含三部分：面部特写、全身正面形象、侧面视角，中性灰摄影棚背景，[角色名]，[适用场景/剧情阶段/服装状态]，[性别]，[脸型][肤色][骨相]，[五官细节]，[发型]，身穿[服装细节]，[配饰/伤势/道具状态]，气质[气质]，真实皮肤纹理，自然毛孔，真实织物，电影级写实光影。排斥：[排斥项]，不要动漫风、不要插画风、不要游戏CG、不要3D建模、不要塑料皮肤、不要过度磨皮"
...
提示词规范：
- 人物：必须写成"真人演员古装定妆参考三视图 / 影视棚拍参考照"，同一位演员同一套造型，画面必须同时包含面部特写、全身正面形象、侧面视角；明确适用场景、剧情阶段、服装/伤势/道具状态、脸型、肤色、骨相、发型、配饰、气质、真实皮肤纹理、自然毛孔、真实织物、电影级写实光影、排斥项
...
- 如果是人物资产，必须保留"面部特写、全身正面形象、侧面视角"三个视图要求，不得只输出单一全身图
...
7. 每个镜头必须写清与前后镜的衔接、起始状态、结束状态、画面方向和连续性约束
8. 每句台词必须写清人物台词、表情、动作、情绪和语气
